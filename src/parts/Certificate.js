          ...
          {certificates.map((item, index) => (
            <div className='col' key={`project-${index}`}>
              <Fade direction='up' delay={300 * index} fraction={1} triggerOnce>
                <figure style={{ width: '100%', overflow: 'hidden', borderRadius: 15 }} className='certificate-figure' onClick={() => props.showModal(item)}>
        ...
        </div>
        {
          props.isLandingPage ? (
            <div className='d-flex align-items-center justify-content-center mt-3 d-md-none'>
              <Button className='btn btn-secondary px-4 btn-light d-flex align-items-center' type='link' href='/certificates'>More Certificate</Button>
            </div>
          ) : ''
        }
      </Fade>
